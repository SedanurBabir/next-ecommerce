...
const NavIcons = () => {
  ...
  return (
    <div className="flex items-center gap-4 xl:gap-6  relative">
        ...
        <Image src='/notification.png' alt='heart' width={22} height={22} className="cursor-pointer"/>
        <div className="relative cursor-pointer" onClick={()=> setIsCartOpen((prev => !prev))}>
        <Image src='/cart.png' alt='cart' width={22} height={22} className="cursor-pointer" />
        <div className="absolute -top-4 -right-4 w-6 h-6 bg-red_ rounded-full text-white text-sm flex items-center justify-center ">2</div>
        </div>
        {isCartOpen &&(
            <div>
                <CartModal/>
